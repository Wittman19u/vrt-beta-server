...
function getPoisCirkwi(params, sqlRequest) {
				...
				db.any(sqlRequest).then(function (dataFromDB) {
					...
					pois.data.data.forEach(poi => {
						...
						db.any(`SELECT id_category, id_cirkwi FROM category_correspondence WHERE id_cirkwi IN (${poi.categories.join(', ')}) AND id_category IS NOT NULL ORDER BY id_cirkwi ASC`).then(function (category) {
							...
							poiDB.updated_at = roadtripController.getStringDateFormatted()
							...
							if (index === undefined) {
								...
								requestsPois.push(db.any('update poi set sourcetype=$1, label=$2, city=$3, latitude=$4, longitude=$5, web=$6, linkimg=$7, description=$8, type=$9, duration=$10,rating=$11, price=$12, category_id=$13, geom=$14, sourcelastupdate=$17, updated_at=$18 WHERE source=$15 AND sourceid = $16 RETURNING id', [poiDB.sourcetype, poiDB.label, poiDB.city, poiDB.latitude, poiDB.longitude, poiDB.web, poiDB.linkimg, poiDB.description, poiDB.type, poiDB.duration, poiDB.rating, poiDB.price, poiDB.category_id, poiDB.geom, poiDB.source, poiDB.sourceid, poiDB.sourcelastupdate, poiDB.updated_at]));
								// }
							}
						})
					});
